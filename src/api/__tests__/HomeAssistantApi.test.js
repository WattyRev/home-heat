...
describe('homeAssistantApi', () => {
    ...
    describe('setTemperature', () => {
        ...
        describe('for full climate rooms', () => {
            ...
            it('sets the hvac mode to heat if the temperature is below the minimum comfort temperature', () => {
                expect.assertions(1);
                homeAssistantApi.setTemperature('living_room', 55);
                expect(mockFetch).toHaveBeenCalledWith(
                    'https://wattyha.duckdns.org:1111/api/services/climate/set_hvac_mode',
                    {
                        contentType: 'application/json',
                        headers: { Authorization: 'Bearer home_assistant_token' },
                        method: 'POST',
                        payload: '{"entity_id":"climate.living_room_ac","hvac_mode":"heat"}',
                    }
                );
            });
            it('sets the hvac mode to heat_cool if the temperature is not below the minimum comfort temperature', () => {
                expect.assertions(1);
                ...
                expect(mockFetch).toHaveBeenCalledWith(
                    'https://wattyha.duckdns.org:1111/api/services/climate/set_hvac_mode',
                    {
                        ...
                        headers: { Authorization: 'Bearer home_assistant_token' },
                        method: 'POST',
                        payload: '{"entity_id":"climate.living_room_ac","hvac_mode":"heat_cool"}',
                    }
                );
            });
        });
